...
public static class SocketMessageHelper
{
    ...
    public static void SendMessage(Socket socket, string message)
    {
        ...
        Console.WriteLine($"[Отправка] Подготовка сообщения: \"{message}\"");
        ...
        try
        {
            ...
            Console.WriteLine($"[Отправка] Размер сообщения в байтах: {messageBytes.Length}");
            ...
            Console.WriteLine($"[Отправка] Префикс длины (4 байта): {BitConverter.ToString(lengthPrefix)}");

            // Шаг 3: Отправляем префикс длины
            // Всегда отправляем ровно 4 байта
            int bytesSent = socket.Send(lengthPrefix);
            Console.WriteLine($"[Отправка] Отправлено байт длины: {bytesSent}");

            // Шаг 4: Отправляем само сообщение
            bytesSent = socket.Send(messageBytes);
            Console.WriteLine($"[Отправка] Отправлено байт данных: {bytesSent}");
            Console.WriteLine($"[Отправка] Сообщение успешно отправлено: \"{message}\"");
        }
        catch (SocketException ex)
        {
            Console.WriteLine($"[ОШИБКА] Ошибка сокета при отправке: {ex.SocketErrorCode} - {ex.Message}");
            throw;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[ОШИБКА] Неизвестная ошибка при отправке: {ex.Message}");
            throw;
        }
    }
    ...
    public static string ReceiveMessage(Socket socket)
    {
        ...
        Console.WriteLine("[Получение] Начало получения сообщения...");
        ...
        try
        {
            ...
            Console.WriteLine("[Получение] Ожидание получения 4 байт длины сообщения...");
            ...
            while (bytesRead < 4)
            {
                ...
                if (result == 0)
                {
                    Console.WriteLine("[Получение] Соединение разорвано удаленной стороной");
                    throw new SocketException((int)SocketError.ConnectionReset);
                }

                bytesRead += result;
                Console.WriteLine($"[Получение] Прочитано байт длины: {result} (всего: {bytesRead}/4)");
            }
            ...
            Console.WriteLine($"[Получение] Получена длина сообщения: {messageLength} байт");

            // Дополнительная проверка на корректность длины
            if (messageLength <= 0 || messageLength > 10 * 1024 * 1024) // 10 МБ максимум
            {
                Console.WriteLine($"[ОШИБКА] Некорректная длина сообщения: {messageLength} байт");
            ...
            Console.WriteLine($"[Получение] Ожидание получения {messageLength} байт данных...");
            ...
            while (bytesRead < messageLength)
            {
                ...
                if (result == 0)
                {
                    Console.WriteLine("[Получение] Соединение разорвано во время получения данных");
                    throw new SocketException((int)SocketError.ConnectionReset);
                }

                bytesRead += result;
                Console.WriteLine($"[Получение] Прочитано байт данных: {result} (всего: {bytesRead}/{messageLength})");
            }

            // Шаг 4: Преобразуем байты в строку с использованием UTF-8 кодировки
            string message = Encoding.UTF8.GetString(messageBuffer);
            Console.WriteLine($"[Получение] Успешно получено сообщение: \"{message}\"");

            return message;
        }
        catch (SocketException ex)
        {
            Console.WriteLine($"[ОШИБКА] Ошибка сокета при получении: {ex.SocketErrorCode} - {ex.Message}");
            throw;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[ОШИБКА] Неизвестная ошибка при получении: {ex.Message}");
            throw;
        }
    }
}